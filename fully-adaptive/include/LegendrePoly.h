...
static void printMatrix(const matrix_t& matrix) 
{
...
static size_t countNonZeros(const matrix_t& matrix, const real_t eps = 1.0e-12)
{
  return std::count_if(
      matrix.begin(),
      matrix.end(),
...
template<typename matrix_t, typename real_t = typename matrix_t::value_type>
static bool verifyMatrix(const matrix_t& A, const matrix_t& B, const real_t eps = 1.0e-12)
{
  const int N = A.size();
  bool success = true;
  for (int i = 0; i < N; i++)
...
template<typename matrix_t, typename real_t = typename matrix_t::value_type>
static auto invertMatrix(const matrix_t A) -> enableIf<std::is_same<real_t,double>::value, matrix_t>
{
  matrix_t Ainv;
...
template<typename matrix_t, typename real_t = typename matrix_t::value_type>
static std::string generateMatvecCode(const matrix_t matrix, const real_t eps = 1.0e-12)
{
  using namespace std;
  ostringstream code;
