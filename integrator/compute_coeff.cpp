...
//  const auto& res = optimize(p, s, beta_p, h, ev_space);
  ...
  return std::make_tuple(opt.get_solution(), h);
}
...
//  gamma = 0.5;
  ...
  beta_p = gamma;
  beta = _beta*std::pow(stiff_kappa, -gamma+epsilon*(1-gamma));

  real_type alpha_s = 0.15;
  if (p == 4)
    alpha_s = 0.31;
